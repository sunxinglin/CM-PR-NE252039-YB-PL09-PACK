...
public partial class LetGoPage : StationTaskCommonPage
{
    ...
    public ReactiveProperty<Brush> TipColor { get; }
    public ReactiveCommand ReUploadMes { get; }
    public ReactiveCommand SetComplete { get; }
    public ReactiveProperty<string> Tips { get; }

    public LetGoPage(StationTaskDTO mapping, APIHelper apiHelper, IMediator mediator, StationPLCContext stationPLCContext)
    {
        InitializeComponent();
        ...
        this.TipColor = new ReactiveProperty<Brush> { Value = Brushes.DarkGreen };
        ...
        SetComplete = new ReactiveCommand().WithSubscribe(async () =>
        {
            if (!HasUpMesOK.Value)
            {
    ...
    public async void CatchIOMessage(IOEnum io)
    {
        try
        {
            ...
            {
                _logger.LogError($"按放行尝试调用放行AGV LetGoPage");
                App._RealtimePage.Vm.DealRunAGV();
            }
            else
            {
        ...
        {
            await _mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.系统运行错误, Name = nameof(AlarmCode.系统运行错误), Module = AlarmModule.SERVER_MODULE, Description = $"放行AGV时发生系统错误:{ex.Message}\r\n{ex.StackTrace}！" });
        }
    }
    ...
    public async Task UpMes()
    {
        ...
        this.TipColor.Value = Brushes.OrangeRed;
        ...
        if (catlAPIResult)
        {
            ...
            this.TipColor.Value = Brushes.DarkGreen;
        ...
        this.TipColor.Value = Brushes.Red;
    }
