...
namespace Ctp0600P.Client.IO
{
    public class IOBoxBusinessProcess
    {
        ...
        {
            if (!hasDeal && App._RealtimePage != null && App._RealtimePage._VM.Alarms.Count == 0)
            {
            ...
            if (hasDeal)
            {
                ...
                if (!notification.AutoMode && notification.ManualMode && !手动)
                {
                    ...
                    //            manualblot.Show();
                    //        };
                    //        ((CheckPowerViewModel)wincheck.DataContext).ModleName = "ManualMode";
                    //        wincheck.Show();
                    //    });
                    //}
                ...
                {
                    Interlocked.Exchange(ref StartDown, _interval);
                    ...
                    try
                    {
                        ...
                        {
                            await _mediator.Publish(new MessageNotice() { showType = Command.NoticeHelp.Enum.MessageShowType.右下角弹窗, messageType = Command.NoticeHelp.Enum.MessageTypeEnum.警告, MessageStr = "当前尚未处于AGV放行任务下！或正在放行AGV" });
                        }
                    }
                    finally
                    {
            ...
            if (notification.Reset && ResetDown <= 0)
            {
                ...
                Interlocked.Exchange(ref ResetDown, _interval);
                MakesReset(true);
            }
            ...
            _stationPLCContext.Alarm = !hasDeal;//是否报警取决于hasDeal
        }
        ...
        public void MakesReset(bool resetaction)
        {
            try
            {
                ...
                if (App._RealtimePage._VM.Alarms.Count == 0)
                {
                    //await ioBoxApi.OnRedLed(DoStatusEnum.OFF);
                    //await ioBoxApi.OnBuzzer(DoStatusEnum.OFF);
                    hasDeal = true;
