...
namespace Ctp0600P.Client
{
    public class HostStartup
    {
        public static void ConfigureServices(HostBuilderContext context, IServiceCollection services)
        {
            ...
            services.AddDbContext<AsZeroDbContext>(options => options.UseSqlServer(context.Configuration.GetConnectionString("AsZeroDbContext")));
            ...
            //拧紧枪
            //services.AddDesoutterServices(context.Configuration.GetSection("Desoutter"));
            //扫码枪
            ...
            //人工位PLC
            services.AddManualStationPLCServices(context.Configuration.GetSection("PlcConnections"), context.Configuration.GetSection("PlcScanOpts"));
            ...
            {
                option.AddPolicy("clientHub", policy => policy.AllowAnyHeader().AllowAnyMethod().AllowCredentials().WithOrigins("https://localhost:7223"));
            });
        }
        ...
        private static void AddDataProtection(HostBuilderContext context, IServiceCollection services)
        {
            ...
            var path = string.IsNullOrEmpty(keyloc)
                ? Directory.GetParent(Assembly.GetExecutingAssembly().Location)
                : new DirectoryInfo(keyloc);
