...
namespace Ctp0600P.Client.ViewModels.StationTaskViewModels
{
    public class StewingTimeoutViewModel : TaskViewModelBase
    {
        ...
        public StewingTimeoutViewModel(StationTaskDTO stationTaskDTO, APIHelper apiHepler, IMediator mediator)
        {
        ...
        public void BindHisData(Proc_StationTask_Record record)
        {
            ...
            {
                mediator.Publish(new AlarmSYSNotification() { Code = AlarmCode.系统运行错误, Name = AlarmCode.系统运行错误.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"绑定静置时长检测历史数据错误:{ex.Message}\r\n{ex.StackTrace}" });
            }
        }
            ...
            if (string.IsNullOrEmpty(Pass))
            {
                App.Current.Dispatcher.Invoke((Action)(async () =>
                {
                    try
                    {
                        ...
                        var timeResult = await _APIHepler.GetStepTaskEndTime(App.PackBarCode, StepCode);
                        if (timeResult.Code != 200)
                        {
                            await mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.系统运行错误, Name = AlarmCode.系统运行错误.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"未查询到PACK码：{App.PackBarCode}在工位{StepCode}任务信息" });
                        }
                        else
                        {
                            //TaskStewingTime.HasFinish = true;
                            AllowTime = Math.Round(allowTime, 0).ToString() + "分钟";
                            var startTime = timeResult.Result;
                            StewingStartTime = startTime.ToString("yyyy-MM-dd HH:mm:ss");

                            var time = Math.Round((DateTime.Parse(NowTime) - startTime).TotalMinutes, 0);//计算实际时间
                            RealTime = time.ToString() + "分钟";
                            ...
                            if (!checkResult)
                            {
                                await mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.静置时长不足报警, Name = AlarmCode.静置时长不足报警.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"静置时长未通过\r\n管控时长{AllowTime}\r\n实际时长{RealTime}" });
                            }
                            ...
                            var saveResult = await _APIHepler.SaveStewingTimeMessage(TaskStewingTime);
                            ...
                            {
                                await mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.静置时长不足报警, Name = AlarmCode.静置时长不足报警.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"保存静置时长检测数据失败" });
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        await mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.静置时长不足报警, Name = AlarmCode.静置时长不足报警.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"[静置时长检测]{ex.Message}\r\n{ex.StackTrace}" });
                    }
                }));
            }
