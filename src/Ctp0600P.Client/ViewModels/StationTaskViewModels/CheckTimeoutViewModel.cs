...
public class CheckTimeoutViewModel : TaskViewModelBase
{
    ...
    public void BindHisData(Proc_StationTask_Record record)
    {
        ...
        {
            _mediator.Publish(new AlarmSYSNotification() { Code = AlarmCode.系统运行错误, Name = AlarmCode.系统运行错误.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"绑定超时检测历史数据错误:{ex.Message}\r\n{ex.StackTrace}" });
        }
    }
    ...
    public async void CatchTimeMessage()
    {
        try
        {
            ...
            {
                await _mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.超时检测报警, Name = AlarmCode.超时检测报警.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"[超时检测]{timeResult.Message}" });
                return;
            }
            ...
            {
                Pass = "不通过";
                await _mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.超时检测报警, Name = AlarmCode.超时检测报警.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"[超时检测] 时间间隔超出范围，范围：[{minTime}, {maxTime}]，实际时长：{duration}" });
                return;
            }
            ...
            {
                await _mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.超时检测报警, Name = AlarmCode.超时检测报警.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"[超时检测]{response.Message}" });
                return;
            }
        ...
        {
            await _mediator.Publish(new AlarmSYSNotification { Code = AlarmCode.超时检测报警, Name = AlarmCode.超时检测报警.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"[超时检测]{ex.Message}\r\n{ex.StackTrace}" });
        }
