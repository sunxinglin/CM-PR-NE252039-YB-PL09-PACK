    ...
    public class BoltGunViewModel : TaskViewModelBase
    {
        ...
        public void BindHisData(Proc_StationTask_Record record)
        {
            try
            {
                foreach (var screw in StationTaskScrewList)
                {
                    ...
                    {
                        screw.ScrewResultList.Add(new ScrewResult { DeviceNo = screwDetail.DeviceNo.ToString(), ScrewState = screwDetail.ResultIsOK ? "OK" : "NG", ScrewValue = screwDetail.FinalTorque.ToString() });
                    }
                }
            }
            catch (Exception ex)
            {
                _mediator.Publish(new AlarmSYSNotification() { Code = AlarmCode.系统运行错误, Name = AlarmCode.系统运行错误.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"绑定拧紧历史数据失败，请联系管理员！" });
            }
        ...
        public async void CatchBoltGunMessage(BoltGunRequest request)
        {
            try
            {
                ...
                if (Screw == null)
                {
                    await _mediator.Publish(new AlarmSYSNotification() { Code = AlarmCode.拧紧枪错误, Name = AlarmCode.拧紧枪错误.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"收到拧紧完成信息，但找不到设备号DeviceNo={request.DeviceNo} 程序号ProgramNo={request.ProgramNo} 的任务。{Environment.NewLine}任务数据：{JsonConvert.SerializeObject(StationTaskScrewList)}，收到的请求：{JsonConvert.SerializeObject(request)}" });
                    return;
                }
                ...
                Screw.CurScrewValueStr = "扭力:" + request.FinalTorque.ToString() + "N";
                ...
                Screw.CurFinalAngleStr = "角度:" + request.FinalAngle.ToString();
                Screw.CurAngle_MaxStr = "上限:" + request.Angle_Max.ToString();
                Screw.CurAngle_MinStr = "下限:" + request.Angle_Min.ToString();
                ...
                Screw.ResultIsOK = await ValidateTightingData(Screw, request);
                ...
                if (!saveOK)
                {
                    await _mediator.Publish(new AlarmSYSNotification() { Code = AlarmCode.系统运行错误, Name = AlarmCode.系统运行错误.ToString(), Module = AlarmModule.DESOUTTER_MODULE, Description = $"拧紧数据保存失败，请联系管理员！" });
                    return;
                }
                // 拧紧失败，防止继续上使能
                if (!Screw.ResultIsOK)
        ...
        private async Task<bool> ValidateTightingData(Base_StationTaskScrew task, BoltGunRequest request)
        {
            //判断拧紧数据是否异常
