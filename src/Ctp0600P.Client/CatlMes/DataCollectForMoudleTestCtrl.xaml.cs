...
{
    /// <summary>
    /// Interaction logic for DataCollectForMoudleTestCtrl.xaml
    /// </summary>
    public partial class DataCollectForMoudleTestCtrl : UserControl
    {
        public DataCollectForMoudleTestCtrl()
        {
            InitializeComponent();
        }
    }
    ...
    public class DataCollectForMoudleTestCtrlVM
    {
        private readonly CatlMesIniConfigApi _configApi;
        private readonly ICatlMesInvoker _catlMesInvoker;
        private readonly IMediator mediator;

        public DataCollectForMoudleTestCtrlVM(CatlMesIniConfigApi configApi, ICatlMesInvoker catlMesInvoker, IMediator mediator)
        {
...
#pragma warning disable CA1416 // 验证平台兼容性
            this.CmdReload = new ReactiveCommand<PasswordBox>()
                .WithSubscribe(async (passwdBox) =>
                {
                    try
                    {
                        var config = await this._configApi.GetDataCollectForMoudleTestConfig(this.StationCode.Value);
                        if (config != null)
                        {
            ...
            this.CmdSave = new ReactiveCommand<PasswordBox>()
                .WithSubscribe(async passwdBox =>
                {
                    try
                    {
                        ...
                        var x = await this._configApi.SetDataCollectForMoudleTestConfig(config, this.StationCode.Value);
                        if (x.Code == 0)
                        {
...
#pragma warning restore CA1416 // 验证平台兼容性
        ...
        private void BindDtoToVM(DataCollectForMoudleTestConfig section, PasswordBox passwdBox)
        {
        ...
        private DataCollectForMoudleTestConfig MapVMToDto(PasswordBox passwdBox)
        {
            ...
            var inter = new DataCollectForMoudleTestParams
            {
