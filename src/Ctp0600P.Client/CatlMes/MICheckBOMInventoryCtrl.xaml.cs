...
{
    /// <summary>
    /// Interaction logic for MICheckBOMInventoryCtrl.xaml
    /// </summary>
    public partial class MICheckBOMInventoryCtrl : UserControl
    {
        public MICheckBOMInventoryCtrl()
        {
            InitializeComponent();
        }
    }
    ...
    public class MICheckBOMInventoryCtrlVM
    {
        private readonly CatlMesIniConfigApi _configApi;
        private readonly ICatlMesInvoker _catlMesInvoker;
        private readonly IMediator mediator;

        public MICheckBOMInventoryCtrlVM(CatlMesIniConfigApi configApi, ICatlMesInvoker catlMesInvoker, IMediator mediator)
        {
            ...
            this.modeCheckOperation = new ReactiveProperty<bool>();
            ...
            this.category = new ReactiveProperty<ObjectAliasEnum>();
...
#pragma warning disable CA1416 // 验证平台兼容性
            ...
            this.CmdReload = new ReactiveCommand<PasswordBox>()
                .WithSubscribe(async (passwdBox) =>
                {
                    try
                    {
                        var config = await this._configApi.GetMICheckBOMInventoryConfig(this.StationCode.Value);
                        if (config != null)
                        {
                        ...
                        {
                            var notice = new MessageNotice() { messageType = MessageTypeEnum.警告, showType = MessageShowType.右下角弹窗, MessageStr = "当前INI文件不含有装物料校验" };
            ...
            this.CmdSave = new ReactiveCommand<PasswordBox>()
                .WithSubscribe(async passwdBox =>
                {
                    try
                    {
                        ...
                        var x = await this._configApi.SetMICheckBOMInventoryConfig(config, this.StationCode.Value);
                        if (x.Code == 0)
                        {
            ...
            this.CmdTest = new ReactiveCommand<PasswordBox>()
                .WithSubscribe(async passwdBox =>
                {
                    try
                    {
                        ...
                        //else
                        //{
                        //    var notice = new MessageNotice() { messageType = MessageTypeEnum.警告, showType = MessageShowType.右下角弹窗, MessageStr = $"进站失败：{JsonConvert.SerializeObject(reponse)}" };
                        //    await this.mediator.Publish(notice);
                        //}
                    }
                    catch (Exception ex)
        ...
        private void BindDtoToVM(MICheckBOMInventoryConfig section, PasswordBox passwdBox)
        {
            ...
            this.modeCheckOperation.Value = section.InterfaceParams.modeCheckOperation;
        ...
        private MICheckBOMInventoryConfig MapVMToDto(PasswordBox passwdBox)
        {
            ...
            var inter = new MICheckBOMInventoryParams
            {
                ...
                modeCheckOperation = this.modeCheckOperation.Value,
        ...
        public ReactiveProperty<bool> modeCheckOperation { get; }
