...
namespace Yee.WebApi.Controllers.BaseData
{
    ...
    {
        //private readonly TaskUpMESDataService _TaskUpMESDataService;
        ...
        public WPFClientDataController(SaveStationDataService saveStationDataService,
            ...
            //TaskUpMESDataService taskUpMESDataService,
            ILogger<WPFClientDataController> logger, Base_StationTaskScanCollectService base_StationTaskScanCollectService, AutomicCommonService commonService)
        {
            _StationService = stationService;
            _SaveStationDataService = saveStationDataService;
            _logger = logger;
            //_TaskUpMESDataService = taskUpMESDataService;
        ...
        /// </summary>
        [HttpPost]
        public async Task<Response<List<Base_ProResource>>> SaveStationEquipmentConifgData(List<Base_ProResource> resList, ProResourceTypeEnum resourceType, string stationCode, string Operator)
        {
            var result = new Response<List<Base_ProResource>>();
            try
            {
                result = await _SaveStationDataService.SaveStationEquipmentConifgData(resList, resourceType, stationCode, Operator);
            }
            catch (Exception ex)
            {
                result.Code = 500;
        ...
        /// </summary>
        [HttpGet]
        public async Task<Response<List<Proc_StationTask_BlotGunDetail>>> LoadBoltData(int stepId, int stationtaskid, string packCode)
        {
            var result = new Response<List<Proc_StationTask_BlotGunDetail>>();
            try
            {
                List<Proc_StationTask_BlotGunDetail> list = await _StationTask_BlotGunDetailService.LoadBoltData(stepId, stationtaskid, packCode);
                result.Result = list;
                result.Code = 200;
            }
            catch (Exception ex)
        ...
        public async Task<Response<bool>> GetScanCollectInfo(int stepid, string packpn, string scanCollectData)
        {
            ...
            try
            {
                var getScanCollectInfoResult = await _StationTaskScanCollectService.GetScanCollectInfo(stepid, packpn, scanCollectData);
        ...
        [HttpGet]
        public async Task<Response<PackTaskMainDataDTO>> LoadStepPackTaskMainData(string stationCode, string beginTime, string endTime)
        {
        ...
        [HttpGet]
        public async Task<Response> CheckVectorBindAndFlowOrder(string packCode, int vectorCode, string stationCode, string VectorStation, string productPn, string? packOutCode)
        {
            var resp = new Response();
            var result = await _CommonService.CheckVectorBindAndFlowOrder(packCode, vectorCode, stationCode, VectorStation, productPn, packOutCode);
            if (result.IsError)
            {
