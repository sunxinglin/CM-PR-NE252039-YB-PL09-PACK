...
{
    public class HistoryData_APIService
    {
        private readonly ILogger<HistoryData_APIService> _logger;
        public readonly AsZeroDbContext _dBContext;

        public HistoryData_APIService(AsZeroDbContext dBContext, ILogger<HistoryData_APIService> logger
    )
        {
        ...
        public async Task<Response> ClearHisData_PeiFang(int id)
        {
            try
            {
                ...
                var main = await _dBContext.Proc_StationTask_Mains.FirstOrDefaultAsync(o => o.Id == id && !o.IsDeleted);
                if (main != null)
                {
                    main.PeiFang_MD5_ID = 0;
                    _dBContext.SaveChanges();
                }
                return res;
            }
            catch (Exception)
            {
                ...
                return new Response() { Code = 500 ,Message="操作失败"};
            }
