...
public class DealClientTightenStartMiddleware : IWorkMiddleware<ScanContext>
{
    ...
    public DealClientTightenStartMiddleware(ILogger<DealClientTightenStartMiddleware> logger, IMediator mediator,
    ...
    public async Task InvokeAsync(ScanContext context, WorkDelegate<ScanContext> next)
    {
        ...
        for (int i = 0; i < tightenReqs.Count; i++)
        {
            ...
            context.MstMsg.TightenStart.Flag =
                new RequestFlagBuilder(context.MstMsg.TightenStart.Flag).Req(true).Build();
        ...
        if (mstReq && devAck)
        {
            ...
            context.MstMsg.TightenStart.SortNo = 0;
            context.MstMsg.TightenStart.Flag =
                new RequestFlagBuilder(context.MstMsg.TightenStart.Flag).Req(false).Build();
