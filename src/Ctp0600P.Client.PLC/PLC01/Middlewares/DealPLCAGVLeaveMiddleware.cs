...
public class DealPLCAGVLeaveMiddleware : IWorkMiddleware<ScanContext>
{
    ...
    public DealPLCAGVLeaveMiddleware(ILogger<DealPLCAGVLeaveMiddleware> logger, IMediator mediator, ITraceApi traceApi,
    ...
    public async Task InvokeAsync(ScanContext context, WorkDelegate<ScanContext> next)
    {
        ...
        if (req && !ack)
        {
            ...
            {
                Content = $"[AGV离站]收到请求，车号:{agvNo}, 工位:{stationCode}", Level = LogLevel.Information,
            ...
            if (resp.Code != 200)
            {
                await _mediator.Publish(new UILogNotification(new LogMessage
                {
                    Content = $"[AGV离站]调用接口失败，{JsonSerializer.Serialize(resp)}",
            ...
            else
            {
                ...
                await _mediator.Publish(new UILogNotification(new LogMessage
                {
                    Content = $"[AGV离站]完成请求，车号:{agvNo}, 工位:{stationCode}",
