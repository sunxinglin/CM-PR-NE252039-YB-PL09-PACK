...
public class DealClientAGVBindPackMiddleware : IWorkMiddleware<ScanContext>
{
    ...
    public DealClientAGVBindPackMiddleware(ILogger<DealClientAGVBindPackMiddleware> logger, IMediator mediator,
        StationPLCContext manualStationNotifyPLCContext, IConfiguration configuration, ITraceApi traceApi)
    {
    ...
    public async Task InvokeAsync(ScanContext context, WorkDelegate<ScanContext> next)
    {
        try
        {
            ...
            if (req.Req)
            {
                ...
                {
                    Content = $"[AGV绑定]发起请求，{JsonSerializer.Serialize(req)}",
            ...
            {
                //更新绑定数据
                var apiResult = await _traceApi.BingAgv(new BindAgvDTO
                {
                    State = context.MstMsg.AGVBindPack.Behavior == 1
                        ? BingAgvStateEnum.绑定
                        : BingAgvStateEnum.解绑,
                ...
                {
                    await _mediator.Publish(new UILogNotification(new LogMessage
                    { Content = $"[AGV绑定]完成请求", Level = LogLevel.Information, Timestamp = DateTime.Now }));

                    //更新Pending消息
                    context.MstMsg.AGVBindPack.Behavior = 0;
                    context.MstMsg.AGVBindPack.AGVNo = 0;
                    ...
                    context.MstMsg.AGVBindPack.Flag =
                        new RequestFlagBuilder(context.MstMsg.AGVBindPack.Flag).Req(false).Build();
                }
                else
                {
                    //发送日志消息
                    await _mediator.Publish(new UILogNotification(new LogMessage
                    { Content = $"[AGV绑定]{response?.Message}", Level = LogLevel.Error, Timestamp = DateTime.Now }));
                }
            }
        }
        catch (Exception ex)
        {
            //发送日志消息
            await _mediator.Publish(new UILogNotification(new LogMessage
            { Content = $"[AGV绑定]{ex?.Message}", Level = LogLevel.Error, Timestamp = DateTime.Now }));
        }
        await next(context);
    }
}
