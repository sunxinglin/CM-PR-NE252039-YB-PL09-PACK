...
public class DealClientReleaseAGVMiddleware : IWorkMiddleware<ScanContext>
{
    ...
    public DealClientReleaseAGVMiddleware(ILogger<DealClientReleaseAGVMiddleware> logger, IMediator mediator,
    ...
    public async Task InvokeAsync(ScanContext context, WorkDelegate<ScanContext> next)
    {
        ...
        for (int i = 0; i < reqs.Count; i++)
        {
            ...
            await _mediator.Publish(new UILogNotification(new LogMessage
                { Content = $"[放行AGV]发起请求", Level = LogLevel.Information, Timestamp = DateTime.Now }));
        }
        ...
        if (mstReq && devAck)
        {
            ...
            await _mediator.Publish(new UILogNotification(new LogMessage
                { Content = $"[放行AGV]完成请求", Level = LogLevel.Information, Timestamp = DateTime.Now }));
        }
