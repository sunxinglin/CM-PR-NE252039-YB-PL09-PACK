...
public class DealPLCAGVArriveMiddleware : IWorkMiddleware<ScanContext>
{
    ...
    public DealPLCAGVArriveMiddleware(ILogger<DealPLCAGVArriveMiddleware> logger, IMediator mediator,
        ITraceApi traceApi, IConfiguration configuration)
    {
        _mediator = mediator;
        _traceApi = traceApi;
    ...
    public async Task InvokeAsync(ScanContext context, WorkDelegate<ScanContext> next)
    {
        ...
        if (req && !ack)
        {
            ...
            {
                Content = $"[AGV到站]收到请求，车号:{agvNo}, 条码:{packCode}, 工位:{stationCode}", Level = LogLevel.Information,
            ...
            if (agvNo <= 0)
            {
                await _mediator.Publish(new UILogNotification(new LogMessage
                {
                    Content = $"[AGV到站]请求异常，车号:{agvNo}, 条码:{packCode}, 工位:{stationCode}", Level = LogLevel.Error,
                    Timestamp = DateTime.Now
                }));
            }
            else
            {
                //调用相关接口
                var resp = await _traceApi.AGVArrived(new AGVMessage
                    { AgvCode = agvNo, ProductCode = packCode, StationName = stationCode, });
                if (resp.Code != 200)
                {
                    await _mediator.Publish(new UILogNotification(new LogMessage
                    {
                        Content = $"[AGV到站]调用接口失败，{JsonSerializer.Serialize(resp)}", Level = LogLevel.Warning,
                        Timestamp = DateTime.Now
                    }));
                }
                else
                {
                    //确认消息
                    context.MstMsg.AGVArrive.Flag =
                        new ResponseFlagBuilder(context.MstMsg.AGVArrive.Flag).Ack(true).Build();
                    await _mediator.Publish(new UILogNotification(new LogMessage
                    {
                        Content = $"[AGV到站]完成请求，{JsonSerializer.Serialize(resp)}", Level = LogLevel.Warning,
                        Timestamp = DateTime.Now
                    }));
                }
            }
