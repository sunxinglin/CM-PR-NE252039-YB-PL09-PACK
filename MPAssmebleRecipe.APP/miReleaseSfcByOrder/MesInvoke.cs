    ...
    public class MesInvoke:MesInvokeBase
    {
        ...
        public override List<object> GetResult(List<object> objs)
        {
            ...
            try
            {
                ...
                miReleaseSfcWithActivityByShoporderResponse response = webservice.miReleaseSfcWithActivityByShoporder(Requst1);
            ...
            {
                Strb_Log.Append("返回代码").Append(",").Append(retCode).Append(",").Append("返回信息").Append(",").Append(errMsg).Append("SFC").Append(sfcs[0]).Append("\r\n");
        ...
        public override List<object> GetResultSimulation(List<object> objs)
        {
            ...
            Requst.isCarrierType = model.IsCarrierTypeField;
            Requst.modeProcessSFC = model.ReleaseSfcByOrderModeProcessSFCField;
            Requst.ColumnOrRowFirstSpecified = false;
            ...
            Requst1.ReleaseSfcWithActivityRequest = Requst;
            int retCode = 111;
            List<string> SfcArr = new List<string>();
            ...
            try
            {
                ...
                moddCode = "SFC" + DateTime.Now.ToString("yyyyMMddhhmmss");
                SfcArr.Add(moddCode);
            }
            catch (WebException ex)
            {
        ...
        protected StringBuilder ParameterLog(releaseSfcWithActivityRequest request)//数据收集参数
        {
            ...
            temp.Append("ShopOrder=").Append(request.shopOrder.ToString()).Append("ColumnOrRowFirst=").Append(request.ColumnOrRowFirstSpecified);
            return temp;
        }
    }
}
