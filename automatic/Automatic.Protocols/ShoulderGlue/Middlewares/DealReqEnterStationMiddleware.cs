...
namespace Automatic.Protocols.ShoulderGlue.Middlewares
{
        ...
        : HandlePlcRequestMiddlewareBase<DevMsg, MstMsg, DealReqEnterStationWraps, DealReqEnterStationOkWraps, DealReqEnterStationNgWraps>
    {
        ...
        public DealReqEnterStationMiddleware(ShoulderGlueFlusher flusher, ILogger<DealReqEnterStationMiddleware> logger, IMediator mediator, CommonService commonService, IOptionsMonitor<ApiServerSetting> apiServerSetting) : base(flusher, logger, mediator)
        {
        ...
        protected override async Task HandleErrAsync(MstMsg pending, DealReqEnterStationNgWraps err)
        {
            ...
            await RecordLogAsync(LogLevel.Error, $"[{PlcStationOpt.StationCode}]-PLC请求进入工位NG：{err.ErrorCode} - {err.ErrorMessage}");
        }
        ...
        protected override async Task HandleOkAsync(MstMsg pending, DealReqEnterStationOkWraps descriptions)
        {
            ...
            await RecordLogAsync(LogLevel.Information, $"[{PlcStationOpt.StationCode}]-PLC请求进入工位OK");
        }
        ...
        protected override async Task ResetAckAsync(MstMsg pending)
        {
            ...
            await RecordLogAsync(LogLevel.Information, $"[{PlcStationOpt.StationCode}]-处理PLC请求进入工位结束");
        }
