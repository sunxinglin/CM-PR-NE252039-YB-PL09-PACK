...
{
    public class DealReqStartGlueMiddleware
        : HandlePlcRequestMiddlewareBase<DevMsg, MstMsg, DealReqStartGlueWraps, DealReqStartGlueOkWraps, DealReqStartGlueNgWraps>
    {
        private readonly CommonService _commonService;
        private readonly IOptionsMonitor<ApiServerSetting> _ApiServerSetting;
        private readonly bool _isEmptyLoop;

        public DealReqStartGlueMiddleware(ShoulderGlueFlusher flusher, ILogger<DealReqStartGlueMiddleware> logger, IMediator mediator, CommonService commonService, IOptionsMonitor<ApiServerSetting> apiServerSetting) : base(flusher, logger, mediator)
        {
        ...
        protected override async Task<FSharpResult<DealReqStartGlueOkWraps, DealReqStartGlueNgWraps>> HandleArgsAsync(DealReqStartGlueWraps args)
        {
            ...
            if (r.IsError)
            {
                ...
                return new DealReqStartGlueNgWraps() { ErrorCode = errValue.ErrorCode, ErrorMessage = errValue.ErrorMessage, ErrorType = errValue.ErrorType }.ToErrResult<DealReqStartGlueOkWraps, DealReqStartGlueNgWraps>();
            }
