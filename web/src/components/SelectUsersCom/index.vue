        ...
        <el-card shadow="never" class="body-small custom-card" style="height: 100%;">
          ...
          <el-tree :data="orgsTree" :expand-on-click-node="false" default-expand-all
			...
			<div class="flex-item table-box">
				<div class="flex-row" style="align-items: center;" v-if="loginKey === 'loginUser'"
					...
					<div style="text-align: right;padding: 5px 10px;" :title="names" v-if="names"
						class="flex-item ellipsis">选中用户：{{names}}</div>
				</div>
				<el-table ref="multipleTable" height="calc(100% - 52px - 36px)" v-if="loginKey === 'loginUser'"
					...
					style="width: 100%;border-top: 1px solid #e4e4e4;" @row-click="rowClick"
				...
				<el-table ref="multipleTable" height="calc(100%)" v-else :data="tableData.datas" tooltip-effect="dark"
					v-loading="tableData.loading" border style="width: 100%;" @row-click="rowClick"
					@select="handleSelectionUser" @select-all="handleSelectionUser">
					<el-table-column align="center" type="selection" width="55">
					</el-table-column>
            ...
            <template slot-scope="scope">
              <role-users :role-id="scope.row.id"></role-users>
				...
				<el-pagination :background="false" v-if="loginKey === 'loginUser'" layout="prev, pager, next"
					:total="tableData.total" :page-size="tableData.listQuery.limit" @current-change="handlePageSearch"
	...
	export default {
		...
		watch: {
			'tableData.selectUsersC'() {
				this.tableData.selectUsers = this.tableData.selectUsers.filter(user => !this.tableData.selectUsersC.some(
					x => x.id === user.id))
			}
		},
		computed: {
			names() {
				...
				if (this.tableData.selectUsers.length > 0 || this.tableData.selectUsersC.length > 0) {
					names = [...this.tableData.selectUsers, ...this.tableData.selectUsersC].map(item => item.name || item
						.account).join(',')
				}
				return names
			}
		...
		methods: {
			...
			loadData(page) {
				...
				if (this.loginKey === 'loginUser') {
					(!this.partDatas || this.partDatas.length <= 0)
					this.getUserList()
					return
				}
				this.getRoleList()
			...
			getUserList() {
				...
				users.getList(this.tableData.listQuery).then(response => {
					...
					this.tableData.selectUsersC = [...this.tableData.datas].filter(x => this.tableData.selectUsers
						.some(item => item.id === x.id))
					this.setSelectTable()
				})
			},
			handleClose() {
			...
			getRoleList() {
				...
				roles.getList(this.listQuery).then(response => {
					...
					this.tableData.loading = false
					this.tableData.selectTextsC = [...this.tableData.datas].filter(x => this.tableData.selectTexts
						.indexOf(x.name || x.account) > -1).map(item => item.name || item.account)
					this.tableData.selectIdsC = [...this.tableData.datas].filter(x => this.tableData.selectIds
						.indexOf(x.id) > -1).map(item => item.id)
					this.tableData.selectTexts = [...this.tableData.selectTexts].filter(x => !this.tableData
						.selectTextsC.some(y => x === y))
					this.tableData.selectIds = [...this.tableData.selectIds].filter(x => !this.tableData.selectIdsC
						.some(y => x === y))
					this.setSelectTable()
				})
			},
