    ...
    <el-dialog v-el-drag-dialog class="dialog-mini" width="900px" :visible.sync="dialogDetailVisible">
      <div>
        <el-table ref="detailData" :data="detailData" v-loading="ListLoading" row-key="id" style="width: 100%" border
          ...
          <el-table-column label="位置" prop="locate" align="center" width="auto" />
...
import * as pressureData from "@/api/pressure";
...
export default {
  ...
  methods: {
    getlist() {
      ...
      pressureData.LoadPressureData(this.query).then((response) => {
    ...
    ModelExpornt() {
      pressureData.modelExpornt(this.query).then((request) => {
        this.$notify({
          title: "提示",
          message: "数据整理完毕,正在下载",
          type: "success",
    ...
    ViewTaskDetails(val) {
      var incaseId = val.id;
      pressureData
        .LoadPressureDataDetail({ dataId: incaseId })
        .then((response) => {
    ...
    upgluedata() {
      ...
      (this.query.isNeedChangeStatus = true),
        pressureData.upCatlAgain({ packCode: this.query.packCode, stationCode: this.query.stationCode, isNeedChangeStatus: true }).then((response) => {
          if (response.code != 200) {
            this.$notify({
              title: "错误",
              message: response.message,
