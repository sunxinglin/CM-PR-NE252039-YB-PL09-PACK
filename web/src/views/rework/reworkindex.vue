    ...
    <el-col :span="24">
      <el-card shadow="never" class="boby-small" style="height: 100%">
        ...
        <div style="margin-bottom: 10px">
          <el-row :gutter="4">
              <el-col :span="21">
                  <el-input @keyup.enter.native="GetTasks"
                            prefix-icon="el-icon-search"
                            size="small"
                            style="width: 300px"
                            class="filter-item"
                            :placeholder="'PACK编码'"
                            v-model="statusqueryListQuery.key"></el-input>
                  <el-button type="primary"
                             icon="el-icon-plus"
                             size="small"
                             @click="GetTasks">查询</el-button>
        ...
        <div>
          <el-table
            :data="statusqueryList"
            ref="dpTable"
            row-key="id"
            v-loading="statusqueryListLoading"
            border
            fit
            stripe
          ...
          >
            ...
            <el-table-column label="操作" align="center" width="300">
              <template slot-scope="scope">
                ...
                <el-button size="mini" @click="SetCompleteTask(scope.row,1)" v-show="scope.row.status != 2"  type="danger"
                  >设置完工</el-button>
                          <el-button size="mini" @click="SetCompleteTask(scope.row,0)" v-show="scope.row.status == 2"  type="danger"
                  >设置未完工</el-button>
				  
				  <el-button size="mini" @click="ClearnPF(scope.row)"  type="danger">清除历史配方</el-button>
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    ...
    ViewTaskDetails(val) {
	  ...
	  ClearnPF(val){
	      this.$confirm("确定要清除历史配方吗？").then((_) => {
	          rework
	              .clearhisdata_peifang({ id: val.id})
	              .then(() => {
	                  this.$notify({
	                      title: "成功",
	                      message: "已清除成功",
