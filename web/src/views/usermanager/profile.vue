				...
				<el-col :span="24" style="height: 100%;">
					...
					<div class="bg-white" style="height: 50%;">
						<el-card shadow="never" class="body-small" style="height: 100%;overflow:auto;">
							...
							<el-form ref="dataForm" :model="temp" label-position="right" label-width="100px">
								<el-form-item size="small" :label="'新密码'" prop="newpwd">
									<el-input v-model="temp.newpwd" show-password></el-input>
								</el-form-item>
								<el-form-item size="small" :label="' '">
...
<script>
	...
	export default {
		...
		mounted() {
this.temp.account=getUserAccount()
			// this.getUserProfile()
			// this.getOrgTree()
			//  this.getModulesTree()
		},
		methods: {
			...
			changePassword() {
				...
				var param={account:this.temp.account,password:this.temp.newpwd}
						const _this = this
				users.changePassword(param).then(response => {
					_this.$message({
						message: '操作成功',
						type: 'success'
					})
			...
			// 	}
			// 	var _this = this
			// 	users.getUserProfile(params).then(response => {
			// 		debugger
