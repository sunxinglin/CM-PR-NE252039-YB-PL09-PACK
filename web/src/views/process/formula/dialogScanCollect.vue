        ...
        <el-card shadow="never" class="boby-small" style="height: 100%">
          ...
          <div>
            <el-table :data="scandata" ref="dpTable" row-key="id" @row-click="weightrowclick"
              ...
              <el-table-column property="scanCollectName" label="详情名称" align="center" width="160"></el-table-column>
              <!-- <el-table-column property="needValidate" :formatter="formatterBoolean" align="center" label="是否需要服务器校验">
              </el-table-column> -->
              <el-table-column property="codeRule" label="扫码规则" align="center"></el-table-column>
      ...
      <div>
        <el-form :rules="weightRules" ref="weightForm" :model="scan" label-position="right" label-width="100px">
          <el-form-item size="small" :label="'详情名称'">
            <el-input v-model="scan.scanCollectName"></el-input>
          </el-form-item>
          <el-form-item size="small" :label="'扫码规则'" >
            <el-input v-model="scan.codeRule"></el-input>
          </el-form-item>
...
<script>
import * as stationTaskScanCollect from "@/api/stationTaskScanCollect";
  ...
  methods: {
    ...
    handleWeightDelete() {
      ...
      this.$confirm("确定要删除吗？")
        .then((_) => {
          ...
          stationTaskScanCollect.del(params).then(() => {
            this.$notify({
              title: "成功",
              message: "删除成功",
              type: "success",
    ...
    updateWeightData() {
      this.$refs["weightForm"].validate((valid) => {
        if (valid) {
          stationTaskScanCollect.update(this.scan).then((response) => {
    ...
    createWeightData() {
      this.$refs["weightForm"].validate((valid) => {
        if (valid) {
          ...
          stationTaskScanCollect.add(this.scan).then((response) => {
            this.dialogoutscanVisible = false; //编辑框关闭
            this.$notify({
              title: "成功",
              message: "创建成功",
    ...
    reloadWeightData() {
      ...
      stationTaskScanCollect.load({ taskid: this.taskId }).then((response) => {
    ...
    back() {
      this.taskId = 0;
      this.$parent.scanCollectVisiable = false;
