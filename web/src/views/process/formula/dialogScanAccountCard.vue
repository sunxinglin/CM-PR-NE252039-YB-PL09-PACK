        ...
        <el-card shadow="never" class="boby-small" style="height: 100%">
          ...
          <div>
            <el-table :data="scandata" ref="dpTable" row-key="id" @row-click="weightrowclick"
              ...
              <el-table-column property="scanAccountCardName" label="详情名称" align="center" width="160"></el-table-column>
      ...
      <div>
        <el-form :rules="weightRules" ref="weightForm" :model="scan" label-position="right" label-width="100px">
          <el-form-item size="small" :label="'详情名称'">
            <el-input v-model="scan.scanAccountCardName"></el-input>
          </el-form-item>
          <el-form-item size="small" :label="'上传代码'" >
...
<script>
import * as stationTaskScanAccountCard from "@/api/stationTaskScanAccountCard";
  ...
  methods: {
    ...
    handleWeightDelete() {
      ...
      this.$confirm("确定要删除吗？")
        .then((_) => {
          ...
          stationTaskScanAccountCard.del(params).then(() => {
            this.$notify({
              title: "成功",
              message: "删除成功",
              type: "success",
    ...
    updateWeightData() {
      this.$refs["weightForm"].validate((valid) => {
        if (valid) {
          stationTaskScanAccountCard.update(this.scan).then((response) => {
    ...
    createWeightData() {
      this.$refs["weightForm"].validate((valid) => {
        if (valid) {
          ...
          stationTaskScanAccountCard.add(this.scan).then((response) => {
            this.dialogoutscanVisible = false; //编辑框关闭
            this.$notify({
              title: "成功",
              message: "创建成功",
    ...
    reloadWeightData() {
      ...
      stationTaskScanAccountCard.load({ taskid: this.taskId }).then((response) => {
    ...
    back() {
      this.taskId = 0;
      this.$parent.scanAccountCardVisible = false;
