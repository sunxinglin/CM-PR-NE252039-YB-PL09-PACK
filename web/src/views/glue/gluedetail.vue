          ...
          <el-row :gutter="2">
            <el-col :span="21">
              <el-input prefix-icon="el-icon-search" size="small" style="width: 200px; height: 20px" class="filter-item"
                :placeholder="'pack码/箱体码'" v-model="query.packCode">
              </el-input>
              ...
              <el-button type="primary" icon="el-icon-plus" size="small" @click="getlist" style="margin-left: 10px">
                查询
              </el-button>
              <el-button size="small" type="primary" @click="upgluedata">涂胶数据重传</el-button>
            </el-col>
          </el-row>
        </div>
      </el-card>
    ...
    <el-dialog v-el-drag-dialog class="dialog-mini" width="900px" :visible.sync="dialogDetailVisible">
      <div>
        <el-table ref="detailData" :data="detailData" v-loading="ListLoading" row-key="id" style="width: 100%" border
          fit stripe highlight-current-row align="left" :height="tablegeight">
          <el-table-column label="参数名称" prop="paramName" align="center" width="auto" />
          <el-table-column label="位置" prop="glueLocate" align="center" width="auto" />
          <el-table-column label="类型" prop="glueType" align="center" :formatter="getDataType" width="auto" />
          <el-table-column label="数据" prop="value" align="center" width="auto" />
...
import * as gluedetail from "@/api/gluedetail";
...
export default {
  ...
  methods: {
    getlist() {
      ...
      gluedetail.LoadGlueData(this.query).then((response) => {
    ...
    upgluedata() {
      ...
      (this.query.isNeedChangeStatus = true),
        gluedetail.upCatlAgain(this.query).then((response) => {
    ...
    ModelExpornt() {
      gluedetail.modelExpornt(this.query).then((request) => {
        this.$notify({
          title: "提示",
          message: "数据整理完毕,正在下载",
          type: "success",
    ...
    ViewTaskDetails(val) {
      var incaseId = val.id;
      gluedetail.LoadGlueDataDetail({ dataId: incaseId }).then((response) => {
        if (response.code != 200) {
          this.$message({
            message: response.message,
            type: "error",
